...
import { bookingValidationSchema, BookingStepValidation } from '@/lib/validation';
...
export function BookingModal() {
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      const errorMessage = error.errors?.[0]?.message || 'Please fill all required fields correctly';
      setValidationError(errorMessage);
  ...
  const handleDownloadInvoice = () => {
    // Create booking details for download
    const bookingDetails = {
      ...
      price: bookingData.serviceType?.price,
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="sticky bottom-0 bg-white border-t p-4 sm:p-6">
            {validationError && (
              <div className="mb-4">
                <FormError message={validationError} className="justify-center" />
              </div>
            )}
