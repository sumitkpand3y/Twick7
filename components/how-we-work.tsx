...
const ResponsiveScrollWorkflow = () => {
  ...
  const workflowSteps = [
    {
      ...
      description: "Visit www.tweak7.co.in and click 'Schedule Appointment'. Fill out the form with your car make, model, odometer reading, and describe any issues. Submit to request your appointment slot.",
      duration: "2 mins",
    ...
    },
    {
      ...
      description: "Our intelligent system assigns a service pickup slot based on your location and preferences. You'll receive instant confirmation with pickup date and time details.",
      duration: "1 hour",
    ...
    },
    {
      ...
      accentColor: "purple",
      bgPattern: "radial-gradient(circle at 20% 20%, rgba(147, 51, 234, 0.15) 0%, transparent 50%)",
      description: "Our professional driver arrives punctually at your location. We take detailed photos of your vehicle's condition before pickup to ensure transparency and accountability.",
      duration: "30 mins",
      features: ["Punctual service", "Photo documentation", "Professional drivers"]
    },
    {
      id: 4,
      ...
      accentColor: "orange",
      bgPattern: "radial-gradient(circle at 80% 80%, rgba(249, 115, 22, 0.15) 0%, transparent 50%)",
      description: "Your vehicle is safely transported to our state-of-the-art service center. Our reception team logs the vehicle and ensures it's securely stored in our facility.",
      duration: "1 hour",
    ...
    },
    {
      ...
      accentColor: "red",
      bgPattern: "radial-gradient(circle at 50% 50%, rgba(239, 68, 68, 0.15) 0%, transparent 50%)",
      description: "Our certified technicians perform a thorough multi-point inspection. We create detailed videos showing your vehicle's current condition, identifying issues and potential improvements.",
      duration: "45 mins",
    ...
    },
    {
      ...
      accentColor: "yellow",
      bgPattern: "radial-gradient(circle at 30% 70%, rgba(245, 158, 11, 0.15) 0%, transparent 50%)",
      description: "Based on our inspection, we generate a comprehensive quotation with itemized costs for parts and labor. No hidden charges - complete transparency in pricing.",
      duration: "15 mins",
    ...
    },
    {
      ...
      description: "After your approval, our skilled technicians begin work using genuine parts. We document the entire process with before, during, and after photos for complete transparency.",
      duration: "2-6 hours",
    ...
    },
    {
      ...
      accentColor: "teal",
      bgPattern: "radial-gradient(circle at 60% 40%, rgba(20, 184, 166, 0.15) 0%, transparent 50%)",
      description: "Comprehensive quality check ensures everything meets our standards. We send a detailed invoice with service breakdown. Pay securely through our encrypted payment system.",
      duration: "10 mins",
    ...
    },
    {
      ...
      accentColor: "pink",
      bgPattern: "radial-gradient(circle at 40% 60%, rgba(236, 72, 153, 0.15) 0%, transparent 50%)",
      description: "After payment confirmation, we schedule the return delivery. Your freshly serviced vehicle is delivered back to your location with final condition photos and service summary.",
      duration: "30 mins",
  ...
  const handleScroll = useCallback(() => {
    ...
    const direction = currentScroll > (scrollMemoryRef.current.get('lastScroll') || 0) ? 'down' : 'up';
    ...
    stepRefs.current.forEach((ref, index) => {
      if (ref) {
        ...
        const isInView = rect.top < window.innerHeight * centerThreshold && rect.bottom > window.innerHeight * (1 - centerThreshold);
    ...
    stepRefs.current.forEach((ref, index) => {
      if (ref) {
        const rect = ref.getBoundingClientRect();
        const isAboveView = rect.bottom < window.innerHeight * (isMobile ? 0.6 : 0.5);
        const stepId = index + 1;
        
        if (direction === 'down' && isAboveView) {
          newCompletedSteps.add(stepId);
        } else if (direction === 'up' && scrollMemoryRef.current.has(`step_${stepId}`)) {
    ...
    if (direction === 'up') {
      visitedSteps.forEach(stepId => {
        if (!newActiveSteps.has(stepId) && scrollMemoryRef.current.has(`step_${stepId}`)) {
          newCompletedSteps.add(stepId);
        }
      });
    }
      ...
      {isMobile && (
        <>
          <button
            ...
            className="fixed top-4 right-4 z-50 bg-black/30 backdrop-blur-sm rounded-full p-3 text-white"
          >
                    ...
                    return (
                      ...
                      >
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                          isActive ? `bg-gradient-to-br ${step.gradient}` : 'bg-white/10'
                        }`}>
                          <IconComponent className={`w-4 h-4 ${isActive || isCompleted || wasVisited ? 'text-white' : 'text-white/60'}`} />
                        </div>
                        <div className="text-left">
                          <div className={`text-sm font-medium ${isActive || isCompleted || wasVisited ? 'text-white' : 'text-white/60'}`}>
                            {step.shortTitle}
                          </div>
        ...
        <div className="fixed top-4 right-4 bg-black/30 backdrop-blur-sm rounded-2xl px-4 py-2 z-40">
          <div className="flex items-center gap-2 text-white">
            <div className="text-sm font-medium">
              {Math.max(completedSteps.size, visitedSteps.size)}/{workflowSteps.length}
            </div>
            <div className="w-16 h-2 bg-white/20 rounded-full overflow-hidden">
              <div 
                className="h-full bg-gradient-to-r from-blue-400 to-purple-400 transition-all duration-500"
                style={{ width: `${(Math.max(completedSteps.size, visitedSteps.size) / workflowSteps.length) * 100}%` }}
              ></div>
            </div>
          </div>
        </div>
        ...
        <div className="min-h-screen flex flex-col justify-center items-center text-center px-4 md:px-8">
          <div className="mb-8">
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-full px-4 md:px-6 py-2 md:py-3 mb-4 md:mb-6">
              ...
              <span className="text-white font-medium text-sm md:text-base">Premium Car Service Experience</span>
            </div>
            ...
            <p className="text-lg md:text-xl lg:text-2xl text-white/70 max-w-4xl mx-auto leading-relaxed mb-8 md:mb-12 px-4">
              Discover our seamless 9-step process that transforms car servicing into a transparent, convenient, and trustworthy experience.
            </p>
          </div>
          ...
          <p className="text-white/40 mt-4 text-xs md:text-sm">{isMobile ? 'Scroll or tap menu to explore' : 'Scroll to explore each step'}</p>
        </div>

        {/* Steps Timeline - Responsive */}
        <div className="relative px-4 md:px-8">
          {/* Central Timeline - Responsive */}
          <div className={`absolute ${isMobile ? 'left-8' : 'left-1/2 transform -translate-x-1/2'} top-0 bottom-0 w-1 bg-white/20 rounded-full`}></div>
          <div 
            className={`absolute ${isMobile ? 'left-8' : 'left-1/2 transform -translate-x-1/2'} top-0 w-1 bg-gradient-to-b from-blue-400 via-purple-400 to-pink-400 rounded-full transition-all duration-1000`}
            style={{ height: `${(Math.max(completedSteps.size, visitedSteps.size) / workflowSteps.length) * 100}%` }}
          ></div>
          ...
          {workflowSteps.map((step, index) => {
            ...
            return (
              <div 
                ...
                className={`relative ${isMobile ? 'min-h-screen' : 'min-h-screen'} flex items-center py-10 md:py-20`}
              >
                {/* Step Node - Responsive */}
                <div className={`absolute ${isMobile ? 'left-8 transform -translate-x-1/2' : 'left-1/2 transform -translate-x-1/2'} z-20`}>
                  <div 
                    ...
                    style={{ 
                      boxShadow: isActive ? `0 0 30px 5px ${getShadowColor(step.accentColor)}` : ''
                    }}
                  >
                    <IconComponent className={`w-5 md:w-6 lg:w-8 h-5 md:h-6 lg:h-8 transition-all duration-300 ${isActive || isCompleted || wasVisited ? 'text-white' : 'text-white/40'}`} />
                  </div>
                  
                  {/* Step Number - Responsive */}
                  <div className={`absolute -top-1 md:-top-2 -right-1 md:-right-2 w-6 md:w-8 h-6 md:h-8 rounded-full flex items-center justify-center text-xs md:text-sm font-bold transition-all duration-300 ${
                    isActive || isCompleted || wasVisited ? 'bg-white text-gray-900' : 'bg-white/20 text-white/60'
                  }`}>
                    {step.id}
                  </div>
                </div>
                ...
                <div className={`w-full ${isMobile ? 'pl-20 pr-4' : `max-w-lg ${isLeft ? 'mr-auto pr-20' : 'ml-auto pl-20'}`}`}>
                  <div 
                    ...
                    style={{ 
                      ...
                      boxShadow: isActive ? '0 0 30px 5px rgba(255, 255, 255, 0.1)' : ''
                    }}
                  >
                    {/* Header - Responsive */}
                    <div className="p-4 md:p-6 lg:p-8">
                      <div className="flex items-center justify-between mb-3 md:mb-4">
                        <div className={`px-2 md:px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 ${getAccentColorClass(step.accentColor)}`}>
                          <Clock className="w-3 h-3" />
                          {step.duration}
                        </div>
                        <div className={`w-2 md:w-3 h-2 md:h-3 rounded-full ${
                          isCompleted || (wasVisited && !isActive) ? 'bg-green-400' : isActive ? 'bg-yellow-400 animate-pulse' : 'bg-white/20'
                        }`}></div>
                      </div>
                      ...
                      <p className="text-white/70 text-sm md:text-base lg:text-lg mb-4 md:mb-6">{step.shortDesc}</p>
                      
                      <div className={`transition-all duration-700 ${isActive ? 'opacity-100 max-h-96' : 'opacity-0 max-h-0'} overflow-hidden`}>
                        <p className="text-white/60 text-sm md:text-base leading-relaxed mb-3 md:mb-4">{step.description}</p>
                        
                        <div className="flex flex-wrap gap-1 md:gap-2">
                          {step.features.map((feature, idx) => (
                            <span key={idx} className="px-2 md:px-3 py-1 bg-white/10 rounded-full text-xs text-white/80">
                              {feature}
                            </span>
                          ))}
                        </div>
                    ...
                    <div className="h-1 bg-white/10">
                      <div 
                        className={`h-full bg-gradient-to-r ${step.gradient} transition-all duration-1000 ${
                          isCompleted || wasVisited ? 'w-full' : isActive ? 'w-2/3' : 'w-0'
                        }`}
                      ></div>
                    </div>
                  </div>
        ...
        <div className="min-h-screen flex items-center justify-center px-4 md:px-8">
          <div className="text-center">
            <div className="mb-6 md:mb-8">
              ...
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-4 md:mb-6">Ready to Experience Excellence?</h2>
              <p className="text-lg md:text-xl text-white/70 max-w-3xl mx-auto mb-8 md:mb-12">
                Join thousands of satisfied customers who trust us with their vehicles. 
                Experience our transparent, professional, and convenient service today.
              </p>
            </div>
